...
static int upload_texture(SDL_Texture **tex, AVFrame *frame, struct SwsContext **img_convert_ctx) {
    ...
    *img_convert_ctx = sws_getCachedContext(*img_convert_ctx,
    ...
    if (*img_convert_ctx != NULL) {
        uint8_t *pixels[4];
        int pitch[4];
        sws_scale(*img_convert_ctx, (const uint8_t* const *)frame->data, frame->linesize, 0, frame->height, pAVPicture.data, pAVPicture.linesize);
        QImage image(pAVPicture.data[0], frame->width, frame->height, QImage::Format_RGB888);
        ImageShow::pushImage(image);
    } else {
        av_log(NULL, AV_LOG_FATAL, "Cannot initialize the conversion context\n");
        ret = -1;
    }
...
std::deque<QImage> ImageShow::imageQueue = std::deque<QImage>();
ImageShow::ImageShow(QWidget *parent) : QWidget(parent)
{
    viewLable = new QLabel(this);

    avpicture_alloc(&pAVPicture, AV_PIX_FMT_RGB24, default_width, default_height);

    boost::thread imageStreamThread(start);
    boost::thread setImageThread([&](){
        while (true) {
            this->setLabelPix(this->popImage());
        }
    });
}
...
void ImageShow::pushImage(QImage image)
{
...
QImage ImageShow::popImage()
{
    boost::unique_lock<boost::mutex> lock(imageQueue_mtx);
    while (imageQueue.empty()) {
        imageQueue_cv.wait(lock);
    }
    QImage image = imageQueue.front();
